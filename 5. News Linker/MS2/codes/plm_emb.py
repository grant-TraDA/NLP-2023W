#
# PLM-based phrase embedding generation
#
import json
...
with open(file) as f:
    for line in tqdm(f, total=get_num_lines(file)):
        # split the document into a set of sentences
        # for each obtained sentence do
        for sent in sent_tokenize(line.strip().replace(' .', '.')):
            # split sentence into a set of tokens (strings)
            sent_toks = [tok for tok in sent.replace('.', ' .').split(' ') if tok != '']
            # encode the obtained set of tokens
            tok_enc = tokenizer([tok.replace('_', ' ') for tok in sent_toks], add_special_tokens=False)['input_ids']
            # calculate starting index of each token
            indices = [1] + (np.cumsum([len(ids) for ids in tok_enc]) + 1).tolist()
            # [[token11,token12,token13],[token21,token22],...]-> [cls_token,token11,token12,token13,token21,..]
            ...
            for tok in sent_toks:
                # add a new record in the vocabulary
                if tok not in vocab:
                    # save the word with its index number within the corpus
                    vocab[tok] = len(vocab)
