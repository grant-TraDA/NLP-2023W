      ...
      if (line[i] == ' ' || i == read) {
        ...
        if ((vocab_idx = SearchVocab(tmp_word)) != -1) {
          ...
          if (flag == 0) {
            ...
            printf("Added topic %i | name %s | %i | %i\n", num_topic, topics[num_topic].category_name,topics[num_topic].init_size,vocab_idx);
            flag = 1;
          }
        } else {
...
void ExpandTopic() {
  ...
  for (a = 0; a < num_topic; a++) {
    ...
    if (fix_seed) {
      ...
      for (b = 0; b < vocab_size; b++) {
        ...
        for (c = 0; c < topics[a].init_size; c++) {
          if (rankings[b] == topics[a].cur_words[c]) {
            //printf("a=%li | b=%li | rankings[b]=%i | c=%li | topics[a].cur_words[c]=%i\n",a,b,rankings[b],c,topics[a].cur_words[c]);
            flag = 1;
            break;
          }
        }
        // TODO: UPD1
        if (flag == 0) {
          //printf("%i %lli ",num_topic, vocab_size);
          //printf("cur_sz = %i ++ => cur_words update1 - %i\n",cur_sz,rankings[b]);
          topics[a].cur_words[cur_sz++] = rankings[b];
        }
      ...
      // TODO: UPD2
      //printf("EXPAND: %i\n",expand);
      for (b = 0; b < topics[a].cur_size + expand; b++) {
        //printf("cur_words update2\n");
        topics[a].cur_words[b] = rankings[b];
      }
    }
